...
// --- Configuration ---
// Replaced import.meta with placeholders to prevent build errors
...
// --- COMPONENTS (Merged to fix resolution errors) ---
  ...
  useEffect(() => {
    if (isOpen && userId && supabase) {
      const fetchUser = async () => {
        ...
        // Mocking data here based on ID or session for demo resilience.
        // If you have a profiles table, uncomment below:
        /*
        const { data } = await supabase.from('profiles').select('*').eq('id', userId).single();
        if (data) setUserData(data);
        */
        
        // Mock fallback for visual demo
        setTimeout(() => {
          setUserData({
    ...
    <div className={`relative p-6 rounded-3xl bg-slate-900/40 border border-white/5 hover:border-cyan-500/30 transition-all duration-500 hover:bg-slate-900/60 group ${data.isNew ? 'animate-slide-in' : ''}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      ...
      <div className="flex items-center justify-between mb-5 relative z-10">
        <div className="flex items-center gap-4">
          <div 
            ...
            onClick={() => onUserClick && onUserClick(data.authorId)}
          >
             {data.avatarUrl ? (
        ...
        {isSimulatingAI ? (
          ...
          <div className="space-y-4 animate-fade-in">
            <div className={`transition-all duration-500 ${translated ? 'opacity-40 scale-95 origin-left' : 'opacity-100'}`}>
              <p className="text-xl text-slate-200 font-medium leading-relaxed font-light">"{data.questionOriginal}"</p>
            </div>
            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${translated ? 'max-h-32 opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="flex gap-4 pl-4 border-l-2 border-cyan-500 bg-gradient-to-r from-cyan-900/10 to-transparent p-3 rounded-r-xl">
                ...
                <p className="text-lg text-cyan-100 font-medium leading-relaxed">{data.questionTranslated}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between pt-5 border-t border-white/5 relative z-10">
        <button 
          ...
          className={`flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl transition-all ${translated ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'bg-slate-800/50 text-slate-400 hover:text-white hover:bg-slate-700'} ${isSimulatingAI ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <Languages className="w-3.5 h-3.5" />
          {translated ? 'Show Original' : 'AI Translate'}
        </button>
        <div className="flex items-center gap-3">
           <button onClick={() => setIsExpanded(!isExpanded)} className="text-slate-500 hover:text-white transition-colors flex items-center gap-1.5 text-xs font-medium hover:bg-slate-800 p-2 rounded-lg">
             <MessageSquare className="w-4 h-4" /> {data.comments}
           </button>
           <button onClick={() => !isSimulatingAI && setIsExpanded(!isExpanded)} className={`px-5 py-2 text-xs font-bold rounded-xl transition-all shadow-sm disabled:opacity-50 ${isExpanded ? 'bg-indigo-600 text-white shadow-indigo-500/20' : 'bg-slate-100 text-slate-900 hover:bg-white'}`} disabled={isSimulatingAI}>
            ...
            <div className="mb-6 space-y-4">
              ...
              {data.replies.map((reply) => (
                <div key={reply.id} className="bg-slate-950/60 rounded-xl p-4 border border-white/5 flex gap-4">
                  <div 
                    className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-800 overflow-hidden border border-white/10 cursor-pointer"
                    onClick={() => onUserClick && onUserClick(reply.authorId)}
                  >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span 
                        className="text-xs font-bold text-cyan-300 cursor-pointer hover:underline"
                        onClick={() => onUserClick && onUserClick(reply.authorId)}
                      >
                        {reply.author}
                      </span>
            ...
            {!session && (
              <div className="absolute inset-0 z-20 bg-slate-950/90 backdrop-blur-sm flex flex-col items-center justify-center rounded-xl border border-slate-800">
                 ...
                 <div className="flex gap-3">
                   <button onClick={onLoginGoogle} className="flex items-center gap-2 px-4 py-2 bg-white text-slate-900 text-xs font-bold rounded-lg transition-all hover:bg-slate-100"><Mail className="w-3 h-3" /> Google</button>
                   <button onClick={onLoginGithub} className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white text-xs font-bold rounded-lg transition-all hover:bg-slate-700"><Github className="w-3 h-3" /> GitHub</button>
                 </div>
              </div>
            )}
...
const Navbar = ({ onOpenModal, onOpenLeaderboard, onOpenProfile, xp, level, xpProgress, session, onLoginGithub, onLoginGoogle, onLogout }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${isScrolled ? 'bg-slate-950/80 backdrop-blur-xl border-cyan-500/20 py-3 shadow-[0_0_20px_rgba(6,182,212,0.1)]' : 'bg-transparent border-transparent py-5'}`}>
      ...
      {/* Mobile Menu logic omitted for brevity but preserved in structure */}
    </nav>
  );
};

// 4. StartScreen components
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] p-8 animate-scale-in overflow-hidden">
        ...
        {!session ? (
          ...
          <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); setFormData({ title: '', language: 'English', category: 'Technology', details: '' }); }} className="space-y-6">
            ...
            {/* Additional inputs would go here */}
...
const LeaderboardModal = ({ isOpen, onClose, supabase }) => {
  ...
  useEffect(() => {
    if (isOpen && supabase) {
      setLoading(true);
      // Mocking fetch for resilience in demo
      setTimeout(() => {
        setLeaders([
...
const App = () => {
  ...
  const handleAddQuestion = async (formData) => {
    ...
    const newQuestionPayload = {
      ...
      // Attempt to save avatar, DB will ignore if column missing
